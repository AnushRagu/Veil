...
var state = {
  serverConfig: { url: "http://localhost:3001", enabled: true },
...
var HIGH_CONFIDENCE_AUTO_ACTIONS = ["highlight", "scroll", "focus", "wait", "inspect"];
var CONFIRMATION_REQUIRED_ACTIONS = ["click", "type", "select"];
...
function evaluateRiskLevel(action, mappedElement) {
  const type = action.type;
  const label = (mappedElement?.label || "").toLowerCase();
  const role = (mappedElement?.role || "").toLowerCase();
  if (mappedElement?.sensitive || ["password", "private key", "seed phrase", "crypto", "credit card", "cvv", "aadhaar", "pan", "wire transfer"].some((s) => label.includes(s))) {
...
function generateActionExplanation(action, riskLevel, mappedElement) {
...
function validateAction(action, pageMapElements) {
  let mappedElement = void 0;
  ...
  if (targetId) {
    ...
    if (bounds) {
      mappedElement = pageMapElements.find(
        (el) => bounds && el.bounds && el.bounds.x < (bounds.x ?? 0) + (bounds.width ?? 0) && (el.bounds.x ?? 0) + (el.bounds.width ?? 0) > (bounds.x ?? 0) && el.bounds.y < (bounds.y ?? 0) + (bounds.height ?? 0) && (el.bounds.y ?? 0) + (el.bounds.height ?? 0) > (bounds.y ?? 0)
      );
    }
    if (!mappedElement) {
      return {
  ...
  return {
    ...
    explanation,
    mappedElement,
    validationPassed: policy !== "reject" && policy !== "lockout"
  };
}
...
function validatePlan(plan, pageMapElements) {
