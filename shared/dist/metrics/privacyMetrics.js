 ...
 * PLR = Sensitive Information Exposed / Sensitive Information Present
 */
export function calculatePrivacyLeakageRate(sensitiveExposed, sensitivePresent) {
    if (sensitivePresent <= 0)
        return 0.0;
 ...
 * FNR = Unredacted Sensitive Elements / Total Sensitive Elements
 */
export function calculateFalseNegativeRate(unredactedSensitive, totalSensitive) {
    if (totalSensitive <= 0)
        return 0.0;
 ...
 * MER = Irrelevant Elements Pruned / Total Irrelevant Elements
 */
export function calculateMinimizationEfficiencyRate(irrelevantPruned, totalIrrelevant) {
 ...
 */
export function evaluatePayloadPrivacy(transmittedPageMap, groundTruth) {
    const violations = [];
    ...
    for (const element of transmittedElements) {
        ...
        const isExplicitlySensitive = groundTruth.sensitiveLabels.some((s) => labelLower.includes(s.toLowerCase()));
    ...
    const plr = calculatePrivacyLeakageRate(sensitiveExposedCount, groundTruth.totalSensitiveElements);
    const fnr = calculateFalseNegativeRate(unredactedSensitiveCount, groundTruth.totalSensitiveElements);
    const mer = calculateMinimizationEfficiencyRate(irrelevantPruned, groundTruth.totalIrrelevantElements);
    return {
