...
function findSubmitLikeElement(pageMap) {
    return (findElementByLabel(pageMap, ["submit", "send", "confirm", "apply", "save", "next", "continue"]) ||
        findElementsByRole(pageMap, ["button"]).find((el) => el.label.toLowerCase().includes("submit") || el.label.toLowerCase().includes("send")));
}
...
export function createRuleBasedPlan(context) {
    ...
    const hasSensitiveRedactions = redactionManifest.some((r) => ["password", "credit_card", "cvv", "aadhaar", "pan"].includes(r.category));
