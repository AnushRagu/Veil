...
function findElementsContainingText(pageMap: PageMap, text: string): SanitizedElement[] {
...
function findAllElementsByLabel(pageMap: PageMap, keywords: string[]): SanitizedElement[] {
  const lowerKeywords = keywords.map((k) => k.toLowerCase());
  return pageMap.elements.filter(
    (el) =>
      el.visible &&
...
function findSubmitLikeElement(pageMap: PageMap): SanitizedElement | undefined {
  return (
    findElementByLabel(pageMap, ["submit", "send", "confirm", "apply", "save", "next", "continue", "purchase", "checkout", "buy", "proceed"]) ||
...
export function createRuleBasedPlan(context: PlannerContext): ServerPlan {
  ...
  const isKnownIntent =
    isScrollUp || isScrollDown || isSubmitGoal || isFormPrepGoal || isSearchGoal ||
  ...
  if (isScrollUp) {
    ...
    if (!target) {
      target = findElementsByRole(pageMap, ["switch", "checkbox", "button"]).find(
        (el) => el.label.toLowerCase().includes("toggle") || el.label.toLowerCase().includes("switch")
      );
    }
    if (target) {
      actions.push({
    ...
    const fuzzyMatches: SanitizedElement[] = [];
